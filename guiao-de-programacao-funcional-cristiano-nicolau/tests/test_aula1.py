...
#Exercicio 2.3
@mock.patch('aula1.lista_ocorrencias', side_effect = aula1.lista_ocorrencias)
def test_lista_ocorrencias(mock_lista_ocorrencias):
    assert mock_lista_ocorrencias([1, 2, 2, 3, 3, 3, 4, 4, 4, 4]) ==  ([(1, 1), (2, 2), (3, 3), (4, 4)])
    assert mock_lista_ocorrencias([]) ==  ([])
...
@mock.patch('aula1.maior', side_effect = aula1.maior)
def test_maior(mock_maior):
    assert mock_maior([1,2,3,5,4]) == 5
    assert mock_maior.call_count in [5,6]
    assert mock_maior([]) == None

#Exercicio 3.5
@mock.patch('aula1.menor_restantes', side_effect = aula1.menor_restantes)
def test_menor_restantes(mock_menor_restantes):
    assert mock_menor_restantes([1,2,3,0,5]) == (0,[1,2,3,5])
    assert mock_menor_restantes([]) == None
...
#Exercicio 3.7
@mock.patch('aula1.max_min_restantes', side_effect = aula1.max_min_restantes)
def test_max_min_restantes(mock_max_min_restantes):
    assert mock_max_min_restantes([5,4,3,1,6]) == (6,1,[5,4,3])
    assert mock_max_min_restantes.call_count == 5 
    assert mock_max_min_restantes([]) == None

#Exercicio 3.8
@mock.patch('aula1.media_mediana', side_effect = aula1.media_mediana)
def test_media_mediana(mock_media_mediana):
    assert mock_media_mediana([1,2,3,4,5]) == (3.0, 3)
    assert mock_media_mediana.call_count == 5
